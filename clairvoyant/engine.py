...
class Engine:
        ...
        for i in range(self.testStr, self.testEnd):
            ...
            if simulation:
                ...
                self.account.Equity.append(
                    self.account.TotalValue(data.iloc[i]["close"])
                )
            ...
            if not simulation:
                ...
                if prediction == 1:
                    self.totalBuys += 1
                    if (
                        helpers.change(data.iloc[i + 1].open, data.iloc[i + 1].close)
                        > 0
                    ):
                        self.correctBuys += 1
    ...
    def conditions(self):
        ...
        print(
            "Training: {0} -- {1}".format(
                self.data.iloc[self.trainStr].date, self.data.iloc[self.trainEnd].date
            )
        )
        print(
            "Testing:  {0} -- {1}".format(
                self.data.iloc[self.testStr].date, self.data.iloc[self.testEnd].date
            )
        )
    ...
    def visualize(self, name, width=5, height=5, stepsize=0.02):
        ...
        rb = ListedColormap(
            ["#FF312E", "#6E8894"]
        )  # Red = 0 (Negative) / Blue = 1 (Positve)
        Z = self.model.svc.decision_function(np.c_[xx.ravel(), yy.ravel()])
...
class Simulation(Engine):
    ...
    def start(self, data, capital, logic, **kwargs):
        Engine.start(
            self, data, capital=capital, logic=logic, simulation=True, **kwargs
        )
    ...
    def statistics(self):
        ...
        percentchange = helpers.change(
            self.account.InitialCapital, self.account.TotalValue(FinalPrice)
        )
    ...
    def chart(self, name):
        ...
        BaseEquity = [
            Price * Shares for Price in self.data[self.testStr : self.testEnd].open
        ]

        p.line(
            self.data[self.testStr : self.testEnd].date,
            BaseEquity,
            color="#CAD8DE",
            legend="Buy and Hold",
        )
